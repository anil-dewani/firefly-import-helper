...
if any("spawn" in s for s in psutil.Process(os.getpid()).cmdline()) or any(
    "celery" in s for s in psutil.Process(os.getpid()).cmdline()
):
    import django
...
def process_amazon_statement(statement_id):
    log_message_amazon.apply_async(
        ("INFO", "Started Amazon Statement Processing", statement_id)
    )
    statement_file = models.AmazonStatementFile.objects.get(id=statement_id)
    if statement_file.status == "Processing":
        log_message_amazon.apply_async(
            ("INFO", "Extraction mode is amazon", statement_id)
        )
...
def scrape_additional_amazon_data(statement_id):
    ...
    statement_file = models.AmazonStatementFile.objects.get(id=statement_id)
    amazon_orders = models.AmazonOrderDetail.objects.filter(
        statement_file=statement_file
    ...
    if approved_selenoid_url:
        browser = webdriver.Remote(
            command_executor=approved_selenoid_url,
        ...
        for amazon_order in amazon_orders:
            ...
            try:
                ...
                time.sleep(5)
                main_category_text = browser.find_element(
                    "xpath",
                    '//*[@id="wayfinding-breadcrumbs_feature_div"]/ul/li[1]/span/a',
                ).text
                category_list_text = browser.find_element(
                    "xpath", '//*[@id="wayfinding-breadcrumbs_feature_div"]/ul'
                ).text
            except:  # noqa: E722
                main_category_text = ""
                category_list_text = ""
            ...
            log_message_amazon.apply_async(
                (
                    "INFO",
                    "Got Info: "
                    + str(main_category_text)
                    + ">>"
                    + str(category_list_text),
                    statement_id,
                )
            )
    ...
    if statement_file.status == "Processing":
        if statement_file.statement_type == "Axis CC":
            log_message.apply_async(
                ("INFO", "Extraction mode is Axis CC", statement_id)
            )
            statement_lines = []
                        ...
                        if "Dr" in transaction_line[8].strip():
                            ...
                            models.TransactionData(
                                transaction_date=datetime.datetime.strptime(
                                    str(transaction_line[0]).strip(), "%d/%m/%Y"
                                ).date(),
                            ...
                            log_message.apply_async(
                                (
                                    "ERROR",
                                    "Transaction has no debit or credit amount somehow "
                                    + str(transaction_line[8]),
                                    statement_id,
                                )
                            )
            ...
            log_message.apply_async(
                ("INFO", "Extraction mode is Axis Savings", statement_id)
            )
                ...
                try:
                    if transaction_line[3].strip():
                        ...
                        models.TransactionData(
                            ...
                            asset_account="Axis Savings",
                            amount_credit=float(
                                str(transaction_line[4].replace(" ", ""))
                            ),
                            description=str(transaction_line[2]).strip(),
                            statement_file=statement_file,
                            tags="statement-id-" + str(statement_id),
            ...
            log_message.apply_async(
                ("INFO", "Extraction mode is ICICI CC", statement_id)
            )
            statement_lines = list(csv.reader(open(statement_file.statement_file.path)))
            transaction_lines = []
            transaction_logging = False
            ...
            log_message.apply_async(
                ("INFO", "Extraction mode is ICICI Savings", statement_id)
            )
            statement_lines = list(csv.reader(open(statement_file.statement_file.path)))
            transaction_lines = []
            transaction_logging = False
            ...
            for transaction_line in transaction_lines:
                try:
                    if (
                        transaction_line[4].strip()
                        and float(transaction_line[4].replace(",", "").replace(" ", ""))
                        > 0
                    ):
                        # its a debit entry
                        models.TransactionData(
                        ...
                        transaction_line[3].strip()
                        and float(transaction_line[3].replace(",", "").replace(" ", ""))
                        > 0
                    ):
                        # its a credit entry
                        models.TransactionData(
                            ...
                            asset_account="ICICI Savings",
                            amount_credit=float(
                                str(transaction_line[3].replace(" ", ""))
                            ),
                            description=str(transaction_line[2]).strip(),
                            statement_file=statement_file,
                            tags="statement-id-" + str(statement_id),
            ...
            log_message.apply_async(
                ("INFO", "Extraction mode is SBI Savings", statement_id)
            )
            statement_lines = list(
                csv.reader(open(statement_file.statement_file.path), delimiter="\t")
            )
            transaction_lines = []
            transaction_logging = False
            for statement_line in statement_lines:
                ...
                try:
                    ...
                    if parse(transaction_line[0], fuzzy=False):
                        if transaction_line[5].strip():
                            ...
                            models.TransactionData(
                                transaction_date=datetime.datetime.strptime(
                                    str(transaction_line[0]).strip(), "%d %b %Y"
                                ).date(),
...
def process_amazon_transaction(statement_id, transaction_id):
    ...
    if transaction_object.amount_debit:
        if (
            models.AmazonOrderDetail.objects.filter(
                order_date__date=transaction_object.transaction_date
            )
            .filter(order_amount=transaction_object.amount_debit)
            .count()
            > 0
        ):
            log_message.apply_async(
                ("INFO", "Amazon Debit Transaction Mapped", statement_id)
            )
            detected_amazon_order = (
                models.AmazonOrderDetail.objects.filter(
                    order_date__date=transaction_object.transaction_date
                )
                .filter(order_amount=transaction_object.amount_debit)
            ...
            transaction_object.notes = (
                detected_amazon_order.product_title
                + " "
                + detected_amazon_order.product_link
            )
        ...
        if (
            ...
            .filter(order_amount=transaction_object.amount_credit)
            .count()
            > 0
        ):
            log_message.apply_async(
                ("INFO", "Amazon Credit Transaction Mapped", statement_id)
            )
            detected_amazon_order = (
                models.AmazonOrderDetail.objects.filter(
                    order_date__date=transaction_object.transaction_date
...
def generate_csv_file(statement_id):
    ...
    while True:
        if transactions.count() == transactions.filter(status="Processed").count():
            log_message.apply_async(
                (
                    "INFO",
                    "All Transactions Processed : " + str(transactions.count()),
                    statement_id,
                )
            )
            break
            ...
            log_message.apply_async(
                (
                    "WARNING",
                    "Waiting for transactions. Attempt: " + str(attempts),
                    statement_id,
                )
            )
            time.sleep(10)
            if attempts > 60:
                log_message.apply_async(
                    (
                        "ERROR",
                        "Timeout processing transactions. Attempt: " + str(attempts),
                        statement_id,
                    )
                )
    ...
    qs = transactions
    output_file_name = (
        slugify(statement_file.statement_type)
        + "-"
