...
class StatementFile(models.Model):
    created_at = models.DateTimeField(auto_now_add=True)
    statement_file = models.FileField(upload_to='statement_uploads/',blank=True, null=True,validators=[FileExtensionValidator(allowed_extensions=["pdf","csv","xls"])])
...
class AmazonStatementFile(models.Model):
    created_at = models.DateTimeField(auto_now_add=True)
    statement_file = models.FileField(upload_to='amazon_statement_uploads/', validators=[FileExtensionValidator(allowed_extensions=['csv'])])
    status = models.CharField(max_length=250, choices=STATUS_CHOICES, default="Unprocessed", blank=True, null=True)
    log_data = models.ManyToManyField(ProcessLog, null=True, blank=True)
    selenoid_session_id = models.CharField(max_length=250, default=None, null=True, blank=True)
...
class AmazonOrderDetail(models.Model):
    ...
    main_category = models.CharField(max_length=250, null=True, blank=True, default=None)
    category_list = models.CharField(max_length=250, null=True, blank=True, default=None)
    json_data = models.TextField(blank=True, null=True,default=None)
    status = models.CharField(max_length=250, choices=STATUS_CHOICES, default='Unprocessed')
    statement_file = models.ForeignKey(AmazonStatementFile, on_delete=models.CASCADE, null=True, blank=True, default=None)    

class TransactionData(models.Model):
    created_at = models.DateTimeField(auto_now_add=True)
    transaction_date = models.DateField()
    transaction_id = models.CharField(max_length=250, blank=True, null=True, default=None)
    asset_account = models.CharField(max_length=250, blank=True, null=True, default=None)
    amount_credit = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True, default=None)
    amount_debit = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True, default=None)
    ...
    status = models.CharField(choices=STATUS_CHOICES, default='Unprocessed', max_length=100)